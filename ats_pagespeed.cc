...
transform_plugin(TSCont contp, TSEvent event, void *edata)
{
  ...
  std::string s_host=ctx->gurl->HostAndPort().as_string();
  ...
  if (ok) {
	std::string s_content_type = get_header(response_header_buf, response_header_loc, "Content-Type");
    ...
    if (ctx->record_in_place && content_type != NULL && !content_type->IsHtmlLike()) {
      ...
      RequestContextPtr system_request_context
        (new AtsRequestContext
         (ctx->server_context->thread_system()->NewMutex(), ctx->server_context->timer(),
          "www.foo.com", 80, "127.0.0.1", txn));
...
process_configuration()
{
  ...
  GoogleString  psolConfDir = GoogleString(TSConfigDirGet()) + "/psol/";

    if ((dir = opendir(psolConfDir.c_str())) != NULL) {
    while ((ent = readdir(dir)) != NULL) {
      ...
      if (ent->d_name[0] == '#')
        continue;
      GoogleString confFileName = psolConfDir + ent->d_name;
      fprintf(stderr, "parse [%s]\n", confFileName.c_str());
      if (!new_config->Parse(confFileName.c_str())) {
        TSError("[ats_pagespeed] Error parsing %s", confFileName.c_str());
      }
    }
    closedir(dir);
  }
